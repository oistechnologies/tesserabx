...
- **cbq** installed and wired on the PostgreSQL-backed provider (the `tesserabx` datasource); the worker entrypoint exists and starts cleanly even though no jobs are defined yet. Caching uses the in-memory CacheBox provider (no Redis).
...
- **Both Docker stacks**:
  - `compose.yaml`: `app`, `worker`, `scheduler`, `db`. `app` attaches to the external network named by `PROXY_NETWORK`. No proxy service, no vector-store service.
...
- **`.env.example`** committed, documenting every variable: `PROXY_NETWORK`, PostgreSQL credentials, CBFS provider selection and Backblaze credentials, file upload constraints (max size, allowed types), database backup retention, AI provider config (provider, credentials, model, with OpenRouter as the documented default), mail settings.
- **GitHub Actions CI**: runs TestBox specs against a disposable PostgreSQL container (with `pgvector`). Green on an empty but correctly wired project.
