...
### 2026-05-22 - Phase 13: Hardening pass

- **AI-off invariant fix (critical).** Phase 7 declared every AiFeatureRegistry entry as `requiresAi : true` and the docstrings said the four UI registries would inherit the AI capability flag from that. An audit found NONE of the UI registries actually consulted the `requiresAi` field; only `capabilityFlag : "aiEnabled"` was checked, which add-ons rarely set. An add-on declaring `requiresAi : true` on a nav entry would have leaked through `isVisible()` when `AI_ENABLED=false`. Added the missing check to `NavigationRegistry`, `AdminPagesRegistry`, `TicketPanelRegistry`, and `DashboardWidgetRegistry` `isVisible()` methods. The `normalize()` for each registry now stores `requiresAi`, and the visibility filter shorts-circuit to false when the entry requires AI and `aiEnabled` is off.
- **Tenancy sweep.** Walked every registry implementation. The 20 manifest-only registries (Navigation, AdminPages, TicketPanel, DashboardWidget, Role, Permission, ChannelAdapter, Action, Trigger, ConditionField, Operator, AiFeature, AiProvider, EmbeddingConsumer, ApiResource, WebhookEvent, NotificationTemplate, NotificationChannel, HelpSection, HelpPage) have ZERO `queryExecute` calls — they store metadata only and cannot violate tenancy. The data-touching paths (CustomFieldsService, EntityExtensionService, HelpSearchService, HelpEmbeddingConsumer) all enforce tenancy correctly at the public boundary.
- **EntityExtensionService bug fix.** `upsertRow` was missing two things uncovered during the hardening Phase13HardeningSpec: (1) it never generated an `id` value for the surrogate PK column the Phase 12 migration introduced, so every insert failed with NOT NULL violation; (2) the inner bind counter used `i &= 1` (string concat) instead of `i += 1` so bind keys were `01, 011, 0111` etc., not `d1, d2, d3`. Same Phase 11 gotcha. Fixed both.
- **EntityExtensionServiceSpec migration**. The Phase 9 spec creates a throwaway `tickets_spec_ext` table whose original schema used the FK column as the PK. The Phase 12 migration shape (surrogate `id` + UNIQUE on FK) changed the convention; the spec was updated to match and a defensive `DROP TABLE IF EXISTS` was added to `beforeAll` so stale schemas from previous runs don't survive.
- **Performance review.** `registry_overrides` already carries 4 indexes (the 2 partial unique + per-registry + per-org); per-request override reads stay O(log n). Manifest-only registries cache in singleton scope after the first call. Help-page markdown re-renders per request (~few ms via Flexmark); not bad enough to optimize this pass, logged as a follow-up.
- **Security review summary**:
  - Help audience filter is enforced server-side at every render path (verified `Home`, `Section`, `Page`, `Search`, `Download` handlers; the `Download.extensions` endpoint specifically rejects non-developer viewers BEFORE streaming the file).
  - Permission gates on UI contributions are honored when set; **finding**: an add-on can ship a UI contribution without `requiredPermission` and the entry will appear on the agent surface for every signed-in agent. Defense-in-depth concern (cbSecurity at the route level still protects the actual handler from anonymous access), but worth flagging. Logged as a follow-up: registration-time WARN log + lint rule for entries that target a privileged surface without a permission gate.
  - API routes' `requiresAuth` field is **documentation-only**; auth is enforced per-action via `ensureAgent()` in each handler. An add-on shipping an API route MUST call `ensureAgent()` manually. Logged as a follow-up: route-level cbSecurity rule for `/api/v1/<addon>/*` paths.
- **`Phase13HardeningSpec` (10/10)** at `tests/specs/Phase13HardeningSpec.bx` proves the `requiresAi` filter through all four UI registries and the cross-org leak null-return on `EntityExtensionService.getRow`. Held conditional on the live `AI_ENABLED` setting so the spec is meaningful regardless of dev env state.
- **Docs cross-references**:
  - [`docs/BUILD-PLAN.md`](BUILD-PLAN.md) header note updated: all 14 extensibility phases (0-13) shipped 2026-05-22; pointer to `docs/EXTENSIONS.md` + `sample-addons/example-sync/`.
  - [`docs/FUTURE-WORK.md`](FUTURE-WORK.md) header note updated with the same shipped-as-of date plus pointer to the extension contract.
  - [`CLAUDE.md`](../CLAUDE.md) "Modules" section gained a new "Add-ons" paragraph documenting the `modules/` install location, the `sample-addons/` in-tree location, the manifest contract, and a reminder that the AI-off invariant + tenant isolation rule apply to add-ons too.
- **Full sweep: 571/0/0 in ~71 seconds** (+9 specs from Phase 12's 562 baseline; all green).

Follow-ups logged for after Phase 13:

1. **Registration-time WARN for UI contributions without a permission gate.** Defense-in-depth lint at NavigationRegistry / AdminPagesRegistry / TicketPanelRegistry / DashboardWidgetRegistry add-entry time.
2. **API auth at the route level.** Add cbSecurity rules for `/api/v1/<addon>/*` paths so `requiresAuth : false` is actually honored as documented (and `requiresAuth : true` doesn't depend on the handler remembering to call `ensureAgent()`).
3. **cfmigrations per-add-on scanning.** Today the example-sync migration lives in `resources/database/migrations/` AND `sample-addons/example-sync/migrations/`. The duplication is the documented workaround.
4. **Help-page markdown render caching.** Mtime-keyed cache so the Flexmark pass doesn't re-run per request.

### 2026-05-22 - Phase 12: Reference add-on, CI integration, complete docs

- **Reference add-on at `sample-addons/example-sync/`** demonstrates every registry and every extension contract that core ships. Stubs for every external call (channel adapter does nothing real, AI feature returns placeholder text, API handler echoes input); what is real is the registration shape. Exercises: navigation (3 menu zones across portal + agent), an admin connection page, a role + 2 permissions, a ticket right-column panel, a dashboard widget, a channel adapter, an automation action with parameter schema, an AI feature, an embedding consumer, 2 REST API routes, 2 webhook event keys, an entity-extension table (`tickets_example_sync`), 2 per-tenant settings (one secret), a notification channel + 2 templates, 2 audit event types, 1 help section + 2 help pages (one agent, one developer audience). The cfmapping is `tesserabx.sample-addons.example-sync`.
- **Discovery wiring**: `Application.bx` and `tests/Application.bx` gained a `/tesserabx/sample-addons` mapping; `config/Coldbox.bx`'s `modulesExternalLocation` now lists `["/tesserabx/modules", "/tesserabx/sample-addons"]`. The test runner at `tests/runner.bxm` was extended to scan `sample-addons/<addon>/tests/specs` so the InstallSpec runs in every `box testbox run` invocation.
- **InstallSpec**: 17 specs walk every core registry and assert the example-sync contributions landed. Includes per-tenant settings (`SettingsRegistry.listDescriptors("example-sync")`), audit event types (`AuditService.listEventTypes()`), and the WireBox resolution of `SyncService@example-sync`. This is the canary that the extension contract has not regressed; CI runs it on every push.
- **Migration handling**: the sample add-on ships its migration at `sample-addons/example-sync/migrations/`, but cfmigrations only scans `resources/database/migrations/`, so the migration is duplicated in the core location with a docblock pointer back to the canonical file. A follow-up extends cfmigrations to follow per-add-on migration folders; until then the duplication is the documented workaround.
- **CI**: `.github/workflows/ci.yaml` now carries a comment noting that the sample-addons specs run as part of `box testbox run` because the runner auto-discovers them. A regression in the extension contract fails the whole pipeline.
- **docs/EXTENSIONS.md polish**: added a Quick Start section at the top (4 steps, "scaffold + edit manifest + run InstallSpec + browser test"), a Contents table-of-contents linking to every chapter, and a new "Reference add-on" section at the end describing the sample. The "What is not yet documented here" stub is gone; every contract is fully documented.
- **Full sweep: 562/0/0 in ~72 seconds** (+17 specs from Phase 11's 545 baseline).

Gotchas logged during this phase:

1. **qb `.primaryKey().references()` chained on one column** doesn't make that column both PK and FK; Postgres errored with "column 'id' named in key does not exist" because qb interpreted the chain as "create a PRIMARY KEY (id)" on the new table. Fixed by using a separate `id` surrogate PK + `ticket_id` FK + unique index on `ticket_id` (the established pattern in every other tenancy-extension table).
2. **Manifest field names must match each registry's expected key exactly**. ChannelAdapterRegistry uses `mapping:`, NotificationChannelRegistry uses `wirebox:`, ActionRegistry uses `parameterSchema:` (not `schema:`); these are not interchangeable. BoxLang's `?:` operator throws on a missing struct key when the registry reads `arguments.raw.someKey ?: []`, so a wrong key name silently kills the registration. Same gotcha as Phase 6.
3. **ChannelAdapterRegistry's `addByMapping` calls `adapter.isPullBased()`** as part of cache priming, and any missing method on the adapter is caught + silently swallowed by the registry's try/catch (writeLog only). Missed-method bugs surface as "adapter not registered" rather than as a class-load error. Every IChannelAdapter implementation must implement `isPullBased()` (push-based stubs return false).
4. **WireBox singletons survive `?fwreinit`**. ChannelAdapterRegistry, ActionRegistry, etc. are singletons; their `variables.cache` survives ColdBox reinit. After editing an adapter class to add a missing method, `?fwreinit` is not enough; a `docker restart tesserabx-app` is required to rebuild the singleton.

### 2026-05-22 - Phase 11: Extendable in-app help module

- New first-party module at `modules_app/help/`. Mounts at `/help` (single URL space for both surfaces; the handlers pick the Agent layout for signed-in agents, the Portal layout otherwise). Routes via `modules_app/help/config/Router.bx` (the inline `variables.routes` array in `ModuleConfig.bx` did not satisfy ColdBox's cross-module routing requirements when we tried mounting `/agent/help/*` separately).
- **3 registries + audience resolver:** `HelpSectionRegistry@help`, `HelpPageRegistry@help`, `HelpAudienceResolver@help`. Sections + pages contributed via the standard `settings.tesserabx.helpSections = [...]` / `settings.tesserabx.helpPages = [...]` manifest pattern. Audience tags are `public | client | agent | developer` with the hierarchy enforced server-side via `HelpAudienceResolver.canSeePage(viewer, page)`. Direct navigation to a developer page by a non-developer renders the same "not available" view as an unknown page — never leaks existence.
- **Search:** `HelpSearchService@help` with AI-on (cosine similarity via Phase 7's embedding consumer registry) and AI-off (substring + keyword match weighted title x4, keywords x2, body x1) paths. Audience filter applied AFTER ranking either way. `HelpEmbeddingConsumer@help` implements the Phase 7 contract; vectors are cached in the singleton's variables scope, so the first search after a reinit triggers a batch of embeddings and subsequent searches reuse the cache.
- **Markdown:** `bx-markdown` runtime module installed and added to `server.json onServerInitialInstall` so fresh container builds get it. The `markdown(text)` BIF wraps Flexmark.
- **Content:**
  - Welcome page + 13 development chapters (the live EXTENSIONS.md chapter-by-chapter split + a Phase-11 specific "Help pages and sections" chapter).
  - 1+ help page per core module: `tickets` (2: queue + submitting), `contacts`, `knowledgebase` (2: finding articles + authoring), `automation`, `sla`, `agent` (signing-in + MFA), `portal` (account + MFA).
- **EXTENSIONS.md download:** `/help/download/extensions.md` streams the live `docs/EXTENSIONS.md`, gated to `developer` audience. Surfaced from three places per request: small download button on the home browse card for the Development section, primary button at the top of the Development section landing, info-bar footer on every development chapter page.
- **Permission + role + nav:** `help.developer` permission registered via `settings.tesserabx.permissions`; auto-granted to `agent-admin` in `RoleRegistry`. "Help" main-menu nav entries on both `portal/main` and `agent/main` zones, both pointing at `/help`. Portal's cbSecurity whitelist extended to allow `/help` for anonymous visitors.
- **Global syntax highlighting (cross-cutting):** highlight.js (atom-one-dark) + highlightjs-copy plugin wired into both layouts via jsDelivr CDN (matching the project's existing pattern for Bootstrap / AdminLTE). Auto-runs `hljs.highlightAll()` on every page across both surfaces, so any KB article, ticket message body, help page, or admin doc that emits ` ```lang ` fenced blocks gets colorized automatically. **Gotcha:** highlight.js 11.x dropped the `coldfusion` language and never had `boxlang`; aliased `boxlang`, `bx`, `bxs`, `cfscript`, `cfm`, `cfml` to `javascript` instead — visual match is good (struct literals, strings, var, function, comments all line up).
- **Bug fix uncovered:** `NavigationRegistry` had a latent `struct & struct` (BoxLang treats `&` as string concat) on the manifest-merge path that only fired now that the help module is the first contributor to declare a pre-set nav `id`. Fixed in [NavigationRegistry.bx](../modules_app/core/models/NavigationRegistry.bx).
- **Docs:** new "Help pages and sections" section in `docs/EXTENSIONS.md` covering manifest declaration, audience model, EXTENSIONS download, and the public extension contract.
- **Specs:** `HelpRegistriesSpec` (16/16) covers section + page registry contracts, audience resolver, and the developer-page filter. `HelpSearchSpec` (6/6) exercises the text-match path (the spec suite runs with AI forced off via the `_tesserabx_test_mode` flag).
- **Full sweep: 545/0/0 in ~75 seconds** (+23 specs from Phase 10's 522 baseline).

Gotchas logged during this phase:

1. **`var url = ...` reserved-scope clobber** (re-encountered): My initial help handlers used `var url = arguments.event.getCurrentRoutedURL()` to detect the agent surface. Same pre-existing bug pattern as Phase 10's SlackChannel and MyAccount.bx. Renamed to `var currentUrl`. **Then I removed the URL-sniff entirely**: the handler now picks the layout from `tbxViewer().userType == "agent"` because both surfaces share the same `/help` URL space.
2. **`&=` vs `+=` in BoxLang**: `dot &= av * bv` is STRING concatenation, not numeric accumulation. `HelpSearchService.cosineSimilarity` produced megabyte-long stringified dot products on every search request. Fixed all numeric accumulators to `+=`.
3. **ColdBox cross-module event syntax in module routers**: `route("help").to("help:Home.index")` works from the **top-level** `config/Router.bx` but not from a module's `config/Router.bx`. ColdBox interpreted the `help:Home.index` literal as a single handler reference and looked for a view at `/app/coldbox/system/web/help:Home/index`. Reverted to a single-URL-space approach (everyone hits `/help` regardless of surface).
4. **highlight.js no longer ships ColdFusion**: 11.x dropped the language; verified by enumerating all 192 language files at the CDN. Workaround: alias to `javascript` for visual purposes.
5. **`sqr()` collision with BIF**: I had a private `sqr()` helper that BoxLang shadowed with its built-in `sqr` BIF (which returns the square, not square root). Inlined the math instead.

### 2026-05-22 - Phase 10: Notifications extensibility (templates + channels)

- `NotificationTemplateRegistry@notifications` ships as the defaults layer for per-event message templates keyed on `(event_key, channel, recipient_type)`. Core seeds the 15 templates that previously lived only in DB migrations (6 inapp + 6 email + 3 slack). The `notification_templates` DB table remains the overrides layer; `NotificationsService.dispatchForEvent` now overlays DB rows on top of registry seeds so admin edits keep winning and fresh installs (or add-on-only events) still dispatch.
- `INotificationChannel` contract published at `modules_app/notifications/models/contracts/INotificationChannel.bx`. `NotificationChannelRegistry@notifications` seeds 3 built-in channels (`inapp`, `email`, `slack`), each extracted into its own class under `models/channels/` (InappChannel is a no-op; EmailChannel wraps cbmailservices+settingsService.applyMailOverride; SlackChannel wraps the Slack/Teams webhook POST). Add-ons contribute via `settings.tesserabx.notificationChannels = [...]` declaring a `wirebox` alias for their implementation.
- `NotificationsService.dispatchForEvent` rewritten to inline-iterate DB rows + uncovered registry entries (the dbCovered set prevents double dispatch) and to route every non-inapp delivery through `channelRegistry.send(channelId, notification)`. The old `sendEmail` / `sendSlack` private methods were deleted along with their dedicated injects (mailService, settingsService, slackWebhookUrl) since those now live in the channel classes.
- `docs/EXTENSIONS.md`: new "Notification templates" and "Notification channels" sections cover manifest declaration, dispatch flow, the contract surface, and the back-compat layer.
- Specs: `NotificationRegistriesSpec` (14/14) — template registry seeds + register/findTemplate/listForEvent + register skip on missing keys + channel registry seeds + isRegistered/findById/resolve/send + 3 built-in channels conforming to the contract. The existing `NotificationsServiceSpec` (11/11) passes unchanged, proving the dispatch refactor is back-compat.
- Full sweep: **522/0/0 in 69 seconds**. That's +27 specs from the Phase 9 baseline (482), and 13 previously-pre-existing dev-env failures eliminated by the dev-env fixes below.

**Dev-env fixes landed alongside Phase 10 (latent issues my refactor surfaced, not Phase 10 regressions):**

- **`chr()` → `char()` in 3 files (12 call sites).** `chr()` is a Lucee/ACF BIF that BoxLang doesn't ship; calls were silently throwing "method does not exist" inside try/catch swallows. Fixed in `MyAccount.bx` (verification-email body), `AddonAssetService.bx` (asset tag joining), and `IMAPPoller.bx` (header splitting). Memory written: see global CLAUDE.md "BoxLang lacks chr()" entry.
- **Reserved-scope-name `var url = ...` renames.** `var url` declarations are silently dropped by BoxLang's scope walker because `url` is a reserved scope; subsequent references resolve to the URL scope (a struct). Renamed in `SlackChannel.bx` (the active one in Phase 10) and `MyAccount.bx` (a pre-existing bug). Same gotcha is documented in the global CLAUDE.md.
- **`s3sdk` was missing.** `cbfs ^1.1.4` declares `s3sdk ^5.1.2` as a transitive dep but the container's `box install` never installed it, so every CBFS write through the B2Provider failed with "class s3sdk.models.AmazonS3 not located". `s3sdk ^5.8.1+119` added to `box.json` `dependencies`; `installPaths.s3sdk` set to `modules/s3sdk/` (was a typoed `cbstreams/s3sdk/`).
- **Test app forced to local CBFS provider.** With s3sdk loaded, the b2 provider actually tries to call Backblaze on every attachment write, adding ~14s per spec. `tests/Application.bx onApplicationStart()` now sets `CBFS_DEFAULT_PROVIDER=local` + `CBFS_PUBLIC_PROVIDER=public-local` as JVM system properties so the test app uses the local disk regardless of how dev's `.env` points the live app.
- **DB cleanup of orphaned spec fixtures.** Specs that create agents and SLA policies rarely tear them down. Found 258 orphaned `@example.com` agents (every unassigned-ticket spec broadcasted notifications to all of them, turning a 3s bundle into a 3min one) and 3 stale `sla-spec-*` / `spec-catchall-*` policies (matched before the test's own catch-all, producing flaky failures). Deleted via SQL. `tests/Application.bx onApplicationStart` now calls `purgeFixtureAgents()` and `purgeFixtureSlaPolicies()` to wipe matching rows at every test-app boot so the cruft cannot rebuild between container restarts.
- **JVM heap bumped 1024 → 4096 in `server.json`.** Container has 7.75GB available. The 1GB cap was tight; cumulative pressure from large test sweeps would hit OOM. After the dev-env cleanup the sweep no longer needs anywhere near 4GB, but the higher cap is a free safety net for future growth.

One new gotcha logged: a TestBox spec asserting against the return value of a function that may return null (e.g. `var row = getRow(...); expect( row ).toBeNull();`) gets the `var x = null` declaration dropped by the BoxLang scope walker, producing "key [row] was not located in any scope" on the assertion line. Workaround: assert directly on the call result without the intermediate variable: `expect( getRow(...) ).toBeNull();`. Same root cause as the `var url` gotcha; affects every `var x = <may-return-null>` pattern.

### 2026-05-21 - Phase 9: Custom fields generalization + entity extension convention

- Three new value tables shipped as migrations (`contact_custom_field_values`, `organization_custom_field_values`, `kb_article_custom_field_values`), each mirroring the shape of the existing `ticket_custom_field_values` (typed columns, FK to the parent with `ON DELETE CASCADE`, unique on `(<entity>_id, definition_id)`).
- `CustomFieldsService@tickets` generalized: `getValuesForTicket(ticketId)` and `setValuesForTicket(ticketId, values)` replaced with `getValuesFor(entityType, entityId)` and `setValuesFor(entityType, entityId, values)`. The schema methods already took an `entityType` parameter; only the value path was hard-coded. New `supportedEntities()` returns the catalog (key + label + valueTable + fkColumn) the admin UI and add-on tooling enumerate. Unknown entity types raise `CustomFieldsService.UnknownEntityType`. The 4 production call sites in `modules_app/agent/handlers/Tickets.bx` were migrated to the new API; no back-compat shims kept (per CLAUDE.md's "no shims when you can just change the code" rule).
- Admin Custom Fields UI (`/agent/admin/custom-fields`) gained an entity-type picker. The URL `?entityType=<key>` selects which catalog the page manages; all CRUD actions inherit the scope. The page now reads `prc.supportedEntities` from `preHandler` so the picker nav-pills render automatically as the catalog grows. The single Phase 4 admin card was kept (description widened); no need to split into four cards.
- `EntityExtensionService@core` published as the sanctioned convention for add-on tables that attach data to a core entity without altering core schema. Methods: `tableNameFor(coreEntity, addonId)`, `fkColumnFor(coreEntity)`, `supportedCoreEntities()`, `getRow(...)`, `upsertRow(...)`, `deleteRow(...)`. Naming rule: `<parent_table>_<safe_addon_id>` (hyphens normalize to underscores). Tenancy is enforced when the caller passes `organizationId`; a row whose `organization_id` is null is surfaced unconditionally (the accountless-ticket rule). Per-request cache lives at `request.tbxExtensionCache`.
- `docs/EXTENSIONS.md`: new "Custom fields" and "Entity extension tables" sections covering service contracts, the entity-type table mapping, the admin UI surface, the recommended add-on migration shape, and the tenancy + per-request caching semantics.
- Specs: `CustomFieldsServiceSpec` gained 3 new specs (supportedEntities lists 4 types, UnknownEntityType raised, full round-trip on a contact row that proves the row lands in `contact_custom_field_values`). `EntityExtensionServiceSpec` ships 10 specs covering catalog shape, name normalization, the invalid-id and unknown-entity-type errors, upsert/get/delete round-trip, second-call overwrite, tenancy enforcement (match + mismatch), and the per-request cache. To exercise reads/writes without depending on a real add-on, the spec creates and drops a throwaway `tickets_spec_ext` table in `beforeAll` / `afterAll`.
- Full sweep: 495/4/9 (+13 from Phase 8 baseline; identical pre-existing CBFS/s3sdk dev-env failures).
- One follow-up: the Quick entity models for Contact, Organization, and Article are still custom-fields-unaware. Reads continue to go through `CustomFieldsService.getValuesFor`. A future polish pass could add `getCustomFieldValues()` shortcuts on each entity; not blocking.
